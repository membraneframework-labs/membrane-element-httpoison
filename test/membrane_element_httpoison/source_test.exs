defmodule Membrane.Element.HTTPoison.SourceTest do
  ...
  test "handle_play/1 should start an async request" do
    ...
    assert_called HTTPoison, :request, [:get, "url", "body", [:hd], [opt: :some, stream_to: _, async: :once]]
  end
  ...
  describe "handle_demand/5 should" do
    ...
    test "return error when stream_next fails" do
      ...
      assert {{:error, reason}, new_state} = @module.handle_demand(:source, 42, :bytes, nil, state)
  ...
  describe "handle_other/2 for message" do
    ...
    test "async status 200 should trigger redemand with streaming false", %{state_streaming: state} do
      msg = %HTTPoison.AsyncStatus{code: 200, id: :ref}
      test_msg_trigger_redemand(msg, state)
    end

    test "async status 206 should trigger redemand with streaming false", %{state_streaming: state} do
    ...
    test "async status 302 should should return error and close connection", %{state_streaming: state} do
    ...
    test "async status 416 should should return error and close connection", %{state_streaming: state} do
    ...
    test "async status with unsupported should return error and close connection", %{state_streaming: state} do
      mock(:hackney, [close: 1], :ok)
    ...
    test "async chunk should produce buffer, update pos_counter and trigger redemand", %{state_streaming: state} do
    ...
    test "async end should send EOS event and remove asyn_response from state", %{state_streaming: state} do
    ...
    test "async redirect should change location and start create new request", %{state_streaming: state} do
      ...
      assert_called HTTPoison, :request, [:get, "url2", "body", [:hd], [opt: :some, stream_to: _, async: :once]]
    end
  end
  ...
  def state_resume_not_live(_) do
    ...
    [state: state, second_response: second_response,
  ...
  def test_reconnect(ctx, tested_call) do
    ...
    assert_called HTTPoison, :request, [:get, "url", "", ^expected_headers, [stream_to: _, async: :once]]
  end
